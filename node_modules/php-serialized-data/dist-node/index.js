...
function _parse(input) {
  ...
  if (tokenIdentifier in exports.PHPTypes.identifierMap) {
    return exports.PHPTypes.identifierMap[tokenIdentifier].build(input);
  } else {
    throw new Error(`Failed to match identifier "${tokenIdentifier}".`);
  }
}

function toJs(object, options = {}, seen = new WeakMap()) {
  if (!object || typeof object !== 'object') return object;
  const objectClass = object.constructor;

  if (!Object.values(exports.PHPTypes.identifierMap).includes(objectClass)) {
    return object;
  }

  if (object instanceof exports.PHPTypes.PHPReference) {
    object = object.value;
  }
  ...
  if (object instanceof exports.PHPTypes.PHPArray || object instanceof exports.PHPTypes.PHPObject) {
...
function parseFixedLengthString(input, openingDelimiter = '"', closingDelimiter = '"') {
  ...
  if (byteCountMatches !== null) {
    ...
    if (input.substr(offset, closingDelimiter.length) === closingDelimiter) {
      ...
      if (parseOptions.fixNulls) {
        ...
        let nullCount;
        const valueStart = byteCountMatches[0].length + openingDelimiter.length; // Check for lambdas. String should have ended one byte early, and the value should start with "lambda_".

        nullCount = 1;

        if (decoder.decode(allBytes.slice(byteCount - nullCount, byteCount - nullCount + closingDelimiter.length + 1)) === closingDelimiter + ';' && /^lambda_\d+$/.test(value.substr(0, value.length - nullCount))) {
        ...
        if (decoder.decode(allBytes.slice(byteCount - nullCount, byteCount - nullCount + closingDelimiter.length + 1)) === closingDelimiter + ';' && value.substr(0, 1) === '*') {
          input = input.replace('*', '\u0000*\u0000');
          let [value] = parseFixedLengthString(input, openingDelimiter, closingDelimiter);
          return [value, offset - nullCount + closingDelimiter.length]; // Original offset to keep everything matched up
...
function makeRegExpClass(regex, valueParser) {
...
(function (PHPTypes) {
    ...
    static parseMap(input, openingDelimiter = '{', closingDelimiter = '}') {
      ...
      if (countMatches !== null) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          offset += key.length; // Keys cannot be referenced
  ...
  class PHPObject extends MappedData {
    ...
    static propertyInfo(propertyName, className) {
      ...
      const protectedRegExp = /^\u0000\*\u0000/; // Also handle double nulls caused by fixing missing nulls
      ...
      for (const [PHPKey, PHPValue] of instance.value.entries()) {
        ...
        if (typeof key === 'string') {
          let {
            accessModifier,
            propertyName
          } = this.propertyInfo(key, instance.className);
          key = propertyName;
...
})(exports.PHPTypes || (exports.PHPTypes = {}));
